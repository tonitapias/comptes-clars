        ...
        <Card className="p-8 shadow-xl border-0">
          ...
          {mode === 'create' && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button className="flex-1" disabled={!inputName || !creatorName} onClick={() => onCreate(inputName, creatorName)}>Començar</Button>
              </div>
            </div>
          )}
          ...
          {mode === 'join' && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="flex gap-2 pt-2">
                <Button variant="ghost" onClick={() => setMode('menu')}>Enrere</Button>
                <Button className="flex-1" disabled={inputCode.length < 3} onClick={() => onJoin(inputCode)}>Entrar</Button>
              </div>
            </div>
          )}
        </Card>
...
// ============================================================================
// --- PANTALLA PRINCIPAL (UI COMPLETA) ---
// ============================================================================
...
function MainApp({ tripData, tripId, onUpdate, onExit, loadingData }) {
  ...
  const copyCode = () => {
    navigator.clipboard.writeText(tripId).then(() => {
      setCopied(true); setTimeout(() => setCopied(false), 2000);
        ...
        // Fallback per si navigator falla
  ...
  const handleSaveExpense = async (e) => {
    ...
    let finalDate = newExpense.date ? new Date(newExpense.date) : new Date();
    finalDate.setHours(12, 0, 0, 0); // Force noon to avoid timezone issues
  ...
  const handleSettleDebt = async () => {
    ...
    const repayment = {
      id: Date.now(), title: `Pagament deute`, amount: settleModalOpen.amount,
      payer: settleModalOpen.from, category: 'transfer', involved: [settleModalOpen.to], date: new Date().toISOString()
    };
  ...
  const handleDeleteUser = (userName) => {
    if (expenses.some(e => e.payer === userName || e.involved.includes(userName))) {
      setConfirmAction({ type: 'info', title: 'Acció no permesa', message: "No pots eliminar aquest usuari perquè té despeses o deutes assignats." });
    } else {
  ...
  const handleUpdateSettings = async () => {
    ...
    d.setHours(12, 0, 0, 0);
    await onUpdate({ name: editTripName, currency: currency, createdAt: d.toISOString() });
  ...
  const openEditModal = (expense) => { 
    setNewExpense({ ...expense, date: expense.date ? new Date(expense.date).toISOString().split('T')[0] : '' }); 
    setEditingId(expense.id); setIsModalOpen(true); 
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24 md:pb-10">
        ...
        <div className="max-w-3xl mx-auto flex justify-between items-start relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <button className="bg-white/20 p-1.5 rounded-lg backdrop-blur-sm hover:bg-white/30 transition-colors" onClick={onExit}><LogOut className="text-indigo-100" size={16} /></button>
      ...
      <Modal isOpen={!!confirmAction} onClose={() => setConfirmAction(null)} title={confirmAction?.title || 'Confirmació'}>
         <div className="space-y-6 text-center"><div className="py-2"><p className="text-slate-600">{confirmAction?.message}</p></div>{confirmAction?.type === 'info' ? <Button onClick={() => setConfirmAction(null)} className="w-full">Entesos</Button> : <div className="flex gap-3"><Button variant="secondary" onClick={() => setConfirmAction(null)} className="flex-1">Cancel·lar</Button><Button variant="danger" onClick={executeConfirmation} className="flex-1" icon={Trash2}>Eliminar</Button></div>}</div>
      </Modal>

      {/* Settle */}
      <Modal isOpen={!!settleModalOpen} onClose={() => setSettleModalOpen(null)} title="Confirmar Pagament">
         {settleModalOpen && <div className="space-y-6 text-center"><div className="py-4"><p className="text-slate-500 text-lg">Confirmar pagament de</p><p className="text-xl font-bold text-slate-800 my-2">{settleModalOpen.from} <ArrowRightLeft className="inline mx-2" size={16}/> {settleModalOpen.to}</p><p className="text-3xl font-black text-indigo-600">{formatCurrency(settleModalOpen.amount)}</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => setSettleModalOpen(null)} className="flex-1">Cancel·lar</Button><Button variant="success" onClick={handleSettleDebt} className="flex-1" icon={CheckCircle2}>Confirmar</Button></div></div>}
      </Modal>

      {/* Add/Edit Expense */}
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingId ? "Editar Despesa" : "Nova Despesa"}>
        <form onSubmit={handleSaveExpense} className="space-y-6">
          <div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-2xl text-slate-400 font-light">{currency?.symbol || '€'}</span><input type="number" step="0.01" placeholder="0.00" autoFocus={!editingId} className="w-full pl-10 pr-4 py-4 bg-slate-50 border-2 border-transparent focus:bg-white focus:border-indigo-500 rounded-2xl text-4xl font-bold text-slate-800 text-center outline-none" value={newExpense.amount} onChange={(e) => setNewExpense({...newExpense, amount: e.target.value})} /></div>
          <div className="flex gap-3"><div className="flex-1"><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Concepte</label><input type="text" placeholder="Sopar..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" value={newExpense.title} onChange={(e) => setNewExpense({...newExpense, title: e.target.value})} /></div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Categoria</label><select className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" value={newExpense.category} onChange={(e) => setNewExpense({...newExpense, category: e.target.value})}>{CATEGORIES.filter(c => c.id !== 'all').map(c => (<option key={c.id} value={c.id}>{c.label}</option>))}</select></div></div>
