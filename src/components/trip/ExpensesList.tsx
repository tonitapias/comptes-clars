...
interface ExpensesListProps {
  ...
  onEditExpense: (e: Expense | null) => void;
  currency: Currency;
}

export default function ExpensesList({ 
  expenses, searchQuery, setSearchQuery, filterCategory, setFilterCategory, onEditExpense, currency 
}: ExpensesListProps) {
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      {expenses.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-3xl border border-dashed border-slate-200">
            ...
            {!searchQuery && filterCategory === 'all' && <Button onClick={() => onEditExpense(null)} className="mx-auto mt-4" icon={Plus}>Afegir Despesa</Button>}
        </div>
      ) : (
        <div className="space-y-3">
            {expenses.map((expense) => { 
                ...
                return (
                  <Card key={expense.id} className={`hover:shadow-md transition-all group ${isTransfer ? 'bg-slate-50' : 'bg-white'}`} onClick={() => onEditExpense(expense)}>
                    <div className="flex items-center p-4 cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="text-xs text-slate-500">{isTransfer ? '→' : '•'} {isTransfer ? expense.involved[0] : (expense.splitType === 'equal' ? `${expense.involved.length} pers.` : (expense.splitType === 'exact' ? 'Exacte' : 'Parts'))}</span>
                        </div>
                      </div>
