          ...
          <div className="flex flex-wrap gap-2">
            {visibleUsers.map(u => (
              ...
              >
                ...
                <div className="w-5 h-5 rounded-full overflow-hidden bg-black/10 dark:bg-white/10 flex items-center justify-center text-[8px]">
                    {/* CORRECCIÓ: Afegit referrerPolicy */}
                    {u.photoUrl ? <img src={u.photoUrl} className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : u.name.charAt(0)}
                </div>
                {u.name}
              </button>
            ))}
                ...
                <div className="space-y-2">
                    {visibleUsers.map(u => (
                        <div key={u.id} className="flex items-center gap-2 bg-white p-2 rounded-lg border border-slate-200 dark:bg-slate-900 dark:border-slate-700 transition-colors">
                             <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-500 dark:text-slate-400 overflow-hidden shrink-0">
                                {/* CORRECCIÓ: Afegit referrerPolicy */}
                                {u.photoUrl ? <img src={u.photoUrl} className="w-full h-full object-cover" referrerPolicy="no-referrer"/> : u.name.charAt(0)}
                             </div>
