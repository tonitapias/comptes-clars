...
export default function ActivityModal({ isOpen, onClose }: ActivityModalProps) {
  ...
  const getActionConfig = (action: LogEntry['action']) => {
    switch (action) {
      case 'create': 
        return { icon: PlusCircle, color: 'text-emerald-100', bg: 'bg-emerald-500', shadow: 'shadow-emerald-500/50', border: 'border-emerald-400' };
      case 'update': 
        return { icon: Edit, color: 'text-amber-100', bg: 'bg-amber-500', shadow: 'shadow-amber-500/50', border: 'border-amber-400' };
      case 'delete': 
        return { icon: Trash, color: 'text-rose-100', bg: 'bg-rose-500', shadow: 'shadow-rose-500/50', border: 'border-rose-400' };
      case 'join': 
        return { icon: UserPlus, color: 'text-blue-100', bg: 'bg-blue-500', shadow: 'shadow-blue-500/50', border: 'border-blue-400' };
      case 'settle': 
        return { icon: CheckCircle2, color: 'text-indigo-100', bg: 'bg-indigo-500', shadow: 'shadow-indigo-500/50', border: 'border-indigo-400' };
      case 'settings': 
        return { icon: Settings, color: 'text-slate-100', bg: 'bg-slate-500', shadow: 'shadow-slate-500/50', border: 'border-slate-400' };
      default: 
        return { icon: Clock, color: 'text-slate-100', bg: 'bg-slate-400', shadow: 'shadow-slate-400/50', border: 'border-slate-300' };
    }
  };
