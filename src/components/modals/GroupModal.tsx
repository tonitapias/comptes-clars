  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Gestió del Grup">
      <div className="bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl flex relative mb-8 border border-slate-200 dark:border-slate-800">
        <button onClick={() => { trigger('light'); setActiveTab('members'); }} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 relative z-10 ${activeTab === 'members' ? 'text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:text-slate-500'}`}>
            <Users size={16} strokeWidth={2.5}/> Membres
        </button>
        <button onClick={() => { trigger('light'); setActiveTab('share'); }} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 relative z-10 ${activeTab === 'share' ? 'text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:text-slate-500'}`}>
                ...
                <div className="w-full space-y-4">
                    <div className="bg-indigo-50/50 dark:bg-indigo-900/10 p-5 rounded-3xl border border-indigo-100 dark:border-indigo-800/50 relative overflow-hidden group">
                        ...
                        <div className="flex gap-2">
                            <div className="flex-1 bg-white/80 dark:bg-slate-900/50 backdrop-blur-sm px-4 py-3 rounded-2xl text-xs font-mono text-slate-600 dark:text-indigo-200 border border-indigo-100/50 dark:border-indigo-800/50 truncate">{shareUrl}</div>
                            <button onClick={handleCopyLink} className={`px-4 rounded-2xl font-bold text-sm transition-all flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 ${copiedLink ? 'bg-emerald-500 text-white shadow-emerald-500/30' : 'bg-indigo-600 text-white shadow-indigo-500/30'}`}>
                    ...
                    <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800">
                        ...
                        <button onClick={handleCopyCode} className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 hover:text-indigo-500 hover:border-indigo-200 dark:hover:border-indigo-800 transition-all shadow-sm hover:shadow-md active:scale-90">
            ...
            <div className="animate-fade-in">
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar pb-4">
                    {activeUsers.map((u: TripUser, idx) => {
                        ...
                        return (
                            <div key={u.id} className="group relative flex items-center justify-between bg-white dark:bg-slate-900/50 p-4 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:shadow-lg hover:border-slate-200 dark:hover:border-slate-700 hover:-translate-y-0.5" style={{ animationDelay: `${idx * 50}ms` }}>
                                        ...
                                        <div className="flex items-center gap-1.5 mb-0.5">
                                            {editingUserId === u.id ? (
                                                <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
                                                    <input type="text" value={tempName} autoFocus onChange={(e) => setTempName(e.target.value)} className="bg-transparent border-none outline-none text-sm font-bold text-slate-800 dark:text-white w-28 px-1" onKeyDown={(e) => { if (e.key === 'Enter') handleSaveName(); if (e.key === 'Escape') setEditingUserId(null); }} />
                                                ...
                                                <div className="flex items-center gap-2 min-w-0">
                                                    ...
                                                    {u.id === adminId && (
                                                        <span className="bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 p-0.5 rounded-md" title="Admin"><Crown size={10} strokeWidth={3} /></span>
                                                    )}
                                                    {(isMe || !u.linkedUid) && (
                                                        <button onClick={() => { trigger('selection'); setEditingUserId(u.id); setTempName(u.name); }} className="text-slate-300 hover:text-indigo-500 transition-colors opacity-0 group-hover:opacity-100">
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                    {!isLinked && currentUser && (
                                        <button onClick={() => handleClaimUser(u.id)} disabled={!!loadingAction} className="relative overflow-hidden px-4 py-2 rounded-xl bg-indigo-600 text-white text-[10px] font-black uppercase tracking-wider shadow-lg shadow-indigo-500/30 hover:bg-indigo-700 active:scale-95 transition-all">
                                            <span className="relative z-10">Sóc jo</span>
                                        </button>
                                    )}
                                    {!isMe && (
                                        <button onClick={() => handleRemoveUser(u.id, u.name)} disabled={!!loadingAction} className="w-9 h-9 flex items-center justify-center rounded-xl text-slate-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 border border-transparent hover:border-rose-100 dark:hover:border-rose-900/30 transition-all" title="Eliminar del grup">
