...
export default function LandingPage({ user }: LandingPageProps) {
  ...
  // 3. Càrrega de dades via Service
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-indigo-100">
      ...
      <div className="w-full max-w-lg relative z-10 flex flex-col gap-6">
            ...
            <div className="p-4 md:p-6">
                ...
                {isGuest && (
                    <div className="flex flex-col gap-8">
                        <div>
                             <p className="text-center text-slate-600 font-medium mb-6 text-base">Inicia sessió per crear i guardar els teus grups de forma segura.</p>
                            <button onClick={handleLogin} disabled={loginLoading} className="group w-full py-5 bg-[#1a1a1a] hover:bg-black text-white font-bold text-lg rounded-2xl shadow-xl shadow-slate-300/50 transition-all transform active:scale-[0.98] flex items-center justify-center gap-4 relative overflow-hidden">
                        ...
                        <div className="border-t border-slate-200 pt-6">
                            {actionState === 'idle' ? (
                                <button onClick={() => setActionState('joining')} className="w-full flex items-center justify-center gap-3 p-4 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-indigo-600 hover:border-indigo-200 transition-all group shadow-sm">
                                    <Users size={22} className="group-hover:scale-110 transition"/><span className="text-base font-bold">Només vull unir-me a un grup</span>
                                </button>
                            ) : (
                                <div className="animate-fade-in bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                                    ...
                                    <form onSubmit={handleQuickAction} className="space-y-3">
                                        <input autoFocus type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition text-lg font-bold text-slate-800 placeholder:font-normal text-center uppercase tracking-widest font-mono" placeholder="XXX-YYY" value={inputValue} onChange={e => setInputValue(e.target.value)}/>
                                        <button disabled={!inputValue || isSubmitting} className="w-full bg-slate-800 text-white py-4 rounded-xl font-bold text-lg hover:bg-black transition flex items-center justify-center gap-2 disabled:opacity-50">
        ...
        {lastTripId && isGuest && (
             <div onClick={() => navigate(`/trip/${lastTripId}`)} className="mt-4 mx-2 bg-white/60 backdrop-blur-md p-4 rounded-2xl border border-white shadow-sm flex items-center gap-4 cursor-pointer hover:bg-white hover:shadow-md transition-all group">
                <div className="bg-emerald-100/50 p-3 rounded-xl text-emerald-600 group-hover:bg-emerald-500 group-hover:text-white transition-colors"><History size={24} /></div>
                <div className="flex-1"><p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-0.5">Detectat</p><p className="text-base font-bold text-slate-800">Recuperar sessió anterior</p></div>
